...
35.简述面向对象的三大特性？
    ...
    封装: --- 
        狭义上：--- 将类中的属性与方法定义为私有
            def __func(self): python 会自动转化为 _类名__func()
            父类的私有化属性(方法)，子类无法修改，也不可被继承
...
38.mro是什么？ --- Method Resolution Order，或MRO
    ...
    当编程语言支持继承，属性与方法便可能存在于子类或者父类
    所以在方法调用的时候需要对当前类与其父类进行搜索确定方法的所在位置
    搜索的顺序就是所谓的 方法解析顺序
...
44.isinstance和type的作用
    1.判断对象类型
    2.isinstance 可以判断父类子类的关系， type 不行
"""
...
45.使用with语句的好处是什么
    ...
    # 只有支持上下文管理器的对象才能使用with，
    # 即在对象内实现了两个方法：__enter__()和__exit__()
